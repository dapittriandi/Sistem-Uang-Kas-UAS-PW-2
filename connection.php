...
function editUser($data) {
	...
	if (isset($_FILES['photo']) && $_FILES['photo']['error'] === UPLOAD_ERR_OK) {
		...
		$photoDestination = 'assets/upload/' . $photoName;
	...
	$query = mysqli_query($conn, "UPDATE user SET nama_lengkap = '$nama_lengkap', username = '$username', id_jabatan = '$id_jabatan', jenis_kelamin = '$jenis_kelamin', asal = '$asal', no_hp = '$no_hp', photo = '$photo' WHERE id_user = '$id_user'");
...
function addUser($data) {
    ...
    if (mysqli_fetch_assoc($query)) {
        ...
        // check if password matches with password_verify
        if ($password !== $password_verify) {
            setAlert("Failed to add user!", "Password not same as password verification!", "error");
            return header("Location: user.php");
        } else {
            // hash the password
            ...
            if (isset($_FILES['photo']) && $_FILES['photo']['error'] === UPLOAD_ERR_OK) {
                ...
                if (move_uploaded_file($photoTmp, $photoDestination)) {
                    ...
                    setAlert("Failed to add user!", "File upload error. Error code: $uploadError", "error");
                    return header("Location: user.php");
                }
            } elseif ($_FILES['photo']['error'] !== UPLOAD_ERR_NO_FILE) {
                // Handle the case where there is an upload error other than no file uploaded
                $uploadError = $_FILES['photo']['error'];
                setAlert("Failed to add user!", "File upload error. Error code: $uploadError", "error");
                return header("Location: user.php");
            }

            $query = mysqli_query($conn, "INSERT INTO user VALUES ('', '$nama_lengkap', '$username', '$password', '$id_jabatan', '$jenis_kelamin', '$no_hp', '$asal',  '$photo')");

            if ($query) {
                setAlert("User added successfully!", "User added successfully!", "success");
            } else {
                setAlert("Failed to add user!", "Failed to insert data into the database.", "error");
            }
